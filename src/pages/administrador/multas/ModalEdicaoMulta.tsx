...
const EditarMultaModal: React.FC<EdicaoModalProps> = ({
  ...
  useEffect(() => {
    if (multa) {
      ...
      if (multa.dataInfracao) {
        ...
        setDataInfracao(d.toISOString().split("T")[0]);
        setHoraInfracao(d.toTimeString().split(" ")[0].substring(0, 5));
      }
  ...
  const handleCodigoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value === "" || /^\d*$/.test(value)) {
      if (value.length <= 8) setCodigoInfracao(value);
    }
  };

  const handleAutoInfracaoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value.toUpperCase();
    if (value.length <= 20) setAutoInfracao(value);
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const dataHoraISO = dataHoraObj.toISOString();
      ...
      const dadosMultas = {
        ...
        valorInfracao,
        placaVeiculo,
        dataInfracao: dataHoraISO as any,
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <Paper sx={modalStyle}>
        ...
        {mensagem && <Alert severity={tipoMensagem} onClose={() => setMensagem("")} sx={{ mb: 2 }}>{mensagem}</Alert>}

        <Box component="form" onSubmit={handleSubmit} display="flex" flexWrap="wrap" gap={2}>
          <TextField label="Código da Infração" type="text" inputMode="numeric" value={codigoInfracao} onChange={handleCodigoChange} required sx={{ flex: "1 1 calc(50% - 8px)" }} inputProps={{ maxLength: 8 }} />
          <TextField select label="Classificação" value={classificacao} onChange={(e) => setClassificacao(e.target.value)} required sx={{ flex: "1 1 calc(50% - 8px)" }}>
            {opcoesClassificacao.map((opcao) => <MenuItem key={opcao.value} value={opcao.value}>{opcao.label}</MenuItem>)}
          </TextField>
          ...
          <TextField label="Auto da Infração" type="text" value={autoInfracao} onChange={handleAutoInfracaoChange} required sx={{ flex: "1 1 calc(50% - 8px)" }} inputProps={{ maxLength: 20 }} />
          <TextField label="Data" type="date" value={dataInfracao} onChange={(e) => setDataInfracao(e.target.value)} required sx={{ flex: "1 1 calc(50% - 8px)" }} InputLabelProps={{ shrink: true }} />
          <TextField label="Hora" type="time" value={horaInfracao} onChange={(e) => setHoraInfracao(e.target.value)} required sx={{ flex: "1 1 calc(50% - 8px)" }} InputLabelProps={{ shrink: true }} />

          <Box sx={{ flex: "1 1 100%", mt: 2, p: 2, border: "1px dashed", borderColor: "divider", borderRadius: 2 }}>
            <Typography variant="subtitle2" fontWeight="bold" gutterBottom color="text.primary">BOLETO</Typography>
            <Box display="flex" alignItems="center" gap={2} flexWrap="wrap">
              {arquivoAtual ? (
                <Tooltip title="Clique para remover o arquivo atual">
                  <Chip icon={<Description />} label={extrairNomeArquivo(arquivoAtual)} onDelete={handleRemoverArquivoAtual} color="primary" variant="outlined" sx={{ maxWidth: "100%" }} />
                </Tooltip>
              ) : <Typography variant="body2" color="text.secondary">Nenhum boleto anexado.</Typography>}
              <Button variant="contained" component="label" size="small" startIcon={<CloudUpload />} color={arquivo ? "success" : "inherit"} sx={{ textTransform: "none" }}>
              ...
              {arquivo && <Chip label={`Upload pendente: ${arquivo.name}`} size="small" color="success" onDelete={() => setArquivo(null)} />}
            </Box>
          </Box>
          ...
          <Box display="flex" justifyContent="flex-end" gap={1} mt={3} sx={{ flex: "1 1 100%" }}>
            ...
            <Button type="submit" variant="contained" disabled={loading || uploading} sx={{ textTransform: "none", minWidth: 120 }}>
